...
const ScrollPopup: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Only count if user actually scrolled
      if (Math.abs(currentPosition - lastScrollPosition) > 5) {
        ...
        scrollTimer = setTimeout(() => {
          isScrolling = false;
        }, 2000); // Pause after 2 seconds of no scrolling
      }
    };

    const updateScrollTime = () => {
      if (isScrolling) {
        totalScrollTime += 0.1; // Add 100ms per interval
        
        if (totalScrollTime >= 9 && !hasShown) {
          setIsOpen(true);
          setHasShown(true);
    ...
    scrollInterval = setInterval(updateScrollTime, 100);
